...
QUIET_START=0
QUIET_END=8
# Папка для файлов состояния (дедуп, буфер дайджеста, кэш). На хостингах
# с эфемерной ФС (Railway и т.п.) укажите путь на примонтированный volume,
# иначе история дублей и буфер дайджеста стираются при каждом деплое!
DATA_DIR=/data
```

⚠️ **Про `DATA_DIR` и Railway/эфемерные хостинги:** без постоянного диска
все JSON-файлы состояния (`published_news.json`, `news_urls.json`,
`pending_digest.json`, `gemini_cache.json`, `settings.json`) стираются
при каждом рестарте контейнера. Это ломает дедупликацию (старые новости
могут опубликоваться повторно) и может потерять буфер дайджеста между
циклами. На Railway: **Settings → Volumes → New Volume**, примонтируйте
на любой путь (например `/data`) и задайте `DATA_DIR=/data` в переменных
окружения сервиса. Без volume бот продолжит работать, но история между
деплоями не сохранится.
