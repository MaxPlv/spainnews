...
def rewrite_and_translate_with_gemini(text, is_title=False):
    ...
    if is_title:
        ...
        prompt = "Ты редактор новостного Telegram-канала про жизнь в Испании. Прочитай полный текст статьи и создай краткую выжимку на русском языке (5-7 предложений). Сохрани все ключевые факты, цифры и детали. Не добавляй мнений, только нейтральный пересказ. Верни только текст выжимки, без вводных фраз типа 'Вот краткая выжимка:', без кавычек. Разбей итоговый текст на абзацы, если это допустимо. В конце через пустую строку добавь 3-4 хэш-тэга сформированных на основе текста. Текст статьи: " + text
