...
def rewrite_and_translate_with_gemini(text, is_title=False, max_retries=3):
    """
    Пересказывает и переводит текст с помощью Gemini API с повторными попытками.
    """
    for attempt in range(max_retries):
        try:
            ...
            # SDK возвращает объект с полем .text
            if response.text:
            ...
            if "429" in error_msg or "quota" in error_msg.lower() or "rate" in error_msg.lower():
                wait_time = 60  # Ждём минуту при превышении лимита
                print(f"   🚫 Превышен лимит запросов API. Ожидание {wait_time} секунд...")
                ...
                raise Exception(f"Не удалось обработать текст после {max_retries} попыток: {error_msg}")
...
def main():
    ...
    for idx, news in enumerate(news_items, 1):
        ...
        try:
            print(f"   🔑 API Key: {API_KEY[:10]}... (первые 10 символов)")
            ...
            # Задержка после первого запроса (увеличена с 10 до 15 секунд)
            print(f"   ⏳ Ожидание 15 секунд перед следующим запросом...")
            time.sleep(15)
            ...
            print(f"   📝 Длина текста для обработки: {len(text_to_process)} символов")
            
            rewritten_text = rewrite_and_translate_with_gemini(text_to_process)
            
            # Задержка после второго запроса (увеличена с 10 до 15 секунд)
            if idx < len(news_items):  # Если это не последняя новость
                print(f"   ⏳ Ожидание 15 секунд перед следующей новостью...")
                time.sleep(15)
